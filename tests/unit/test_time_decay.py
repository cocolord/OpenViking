...
from openviking.storage.vectordb.collection.result import parse_remote_search_result
...
def test_builder_and_local_parser_share_cloud_contract():
    ...
    ops = build_time_decay_post_process_ops(weight=0.2, protection="1d", origin=origin)
...
def test_zero_weight_keeps_post_processing_disabled():
    assert build_time_decay_post_process_ops(weight=0.0, protection="not-a-duration") == []


def test_enabled_decay_uses_the_server_owned_curve():
    ops = build_time_decay_post_process_ops(weight=0.2, protection="0")

    addition = ops[0]["addition_score"][0]
    assert addition["scale"] == "7d"
    assert addition["decay"] == 0.5
...
def test_remote_result_does_not_infer_missing_explanation_scores():
    item = parse_remote_search_result(
        {"data": [{"id": "event", "score": 0.6, "ann_score": 0.9}]}
    ).data[0]

    assert item.score == pytest.approx(0.6)
    assert item.origin_score is None
    assert item.addition_score is None
...
def test_post_process_candidate_budget_matches_cloud_default():
    ...
    with pytest.raises(ValueError, match="must not exceed 100000"):
        post_process_input_limit(100_001)
