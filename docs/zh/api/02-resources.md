...
`GET /api/v1/resources/ttl?uri=...` 读取单个文件实际生效的元数据。`PATCH /api/v1/resources/ttl` 接受精确文件 `uri` 和 ISO 8601 格式的 `expires_at`，可调整该存活文件已有的到期时间。目录 URI 会被拒绝；修改后续文件的目录默认值应使用 `PATCH /api/v1/resources/config`。修改文件期限会保留 generation，不改变兄弟文件或目录策略；此入口不恢复已过期数据，也不为历史未纳管资源补设 TTL。

L2 正文、附件及其检索候选到期后立即逻辑不可见，再由异步任务物理清理；物理删除按稳定规则分散到配置的清理抖动窗口内，不会让所有租户集中在 UTC 0 点执行。所有 L0/L1 摘要及其向量均保留，即使目录内的 L2 已全部过期，也保留目录和摘要。TTL 不隐藏或重建这些摘要，摘要中仍可能保留过期正文的信息。清理失败会保留登记并重试。复制、移动与 OVPack 保留逐文件期限；TTL 状态保存在 OV 元数据中，不增加公有云向量字段。

目录 Watch 会继续逐个刷新仍存活的文件。若某文件已过期且源内容未变化，Watch 会跳过它，避免物理清理后被无条件复活；源内容发生变化时，可以按当时生效的策略作为新一代文件重新导入。

resources、events 和 sessions 的物理清理按 UTC 日批次执行，每天 UTC 00:00（北京时间 08:00）推进清理截止时间；之后到期的对象继续保持不可见，等待下一批次。服务每 30 秒推进当日积压，每批最多领取 100 个对象，队列有未消费任务时暂停领取。删除失败仍按各自的退避时间重试，重启后继续，不必再等一天。到期隐藏至物理清理完成期间仍可能计费。清理完成只确认主存储 L2 文件和向量记录已删除；异步备份完成和计费侧扣减尚未纳入确认，保留的摘要仍占用存储。
