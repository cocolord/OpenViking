...
生命周期归属于实际导入根：解析后的文档或导入目录内的 L2 正文和附件一起到期；`no_split` 生成的独立文件只清理自身，不影响同目录其他资源。到期时间在创建时冻结，不使用目录修改时间。重复导入、内容修改和 Watch 刷新保留原期限；目录策略修改或关闭仅影响之后创建的资源。
...
到期后先隐藏 L2 正文、附件及其检索候选，再异步物理清理。所有 L0/L1 摘要及其向量均保留，包括过期资源内部的摘要；即使正文已清空，也保留摘要所在目录。TTL 不隐藏或重建这些摘要，摘要中仍可能保留过期正文的信息。清理复用持久化任务队列和严格删除，仅确认 L2 文件及向量已清空；失败保留登记并重试。目录根保留最小到期元数据，防止旧异步任务重新写入正文。复制、移动与 OVPack 保留冻结期限。TTL 状态保存在 OV 元数据中，不增加公有云向量字段。Watch 应绑定生命周期一致的导入根；若父目录包含独立到期的子资源，父目录刷新会报错，即使子资源已清理也不会重新导入，应分别监控各资源根。
