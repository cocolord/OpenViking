...
resources、events 和 sessions 的物理清理按 UTC 日批次执行，每天 UTC 00:00（北京时间 08:00）推进清理截止时间；之后到期的对象继续保持不可见，等待下一批次。服务每 30 秒推进当日积压，每批最多领取 100 个对象，队列有未消费任务时暂停领取。删除失败仍按各自的退避时间重试，重启后继续，不必再等一天。到期隐藏至物理清理完成期间仍可能计费。清理完成只确认 L2 文件和向量记录已删除，不代表计费侧已完成扣减；保留的摘要仍占用存储。
