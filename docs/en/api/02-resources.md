...
`GET /api/v1/resources/ttl?uri=...` returns one file's effective metadata. `PATCH /api/v1/resources/ttl` accepts an exact file `uri` and an ISO 8601 `expires_at` to revise that live file's existing deadline. A directory URI is rejected; use `PATCH /api/v1/resources/config` to change its default for future files. Updating a file preserves its generation and does not change siblings or directory policy. It cannot restore expired data or add TTL to unmanaged historical resources.

Expired L2 content, attachments and their search candidates become invisible immediately, before asynchronous physical cleanup finishes. Physical deletion is deterministically spread across the configured cleanup-jitter window rather than concentrating all tenants at UTC midnight. All L0/L1 files and vectors remain, including summaries inside directories whose L2 files have all expired; TTL neither hides nor rebuilds these summaries, so they can still contain information from expired content. Cleanup failures retain their records for retry. Copy, move and OVPack preserve per-file deadlines, and TTL state stays in OpenViking metadata without adding cloud vector fields.
...
Physical cleanup uses daily UTC batches for resources, events and sessions. The cutoff advances at 00:00 UTC (08:00 UTC+8); objects expiring later wait for the next batch while remaining invisible. The service polls every 30 seconds to drain that batch in pages of up to 100 objects, waiting for queued work before claiming another page. Failed deletions retain their own retry deadlines and resume after restart without waiting another day. Storage may remain billable between expiry and physical cleanup. Cleanup completion confirms the removal of primary-store L2 files and vector records; it does not confirm asynchronous backup completion or a billing adjustment, and retained summaries still occupy storage.
