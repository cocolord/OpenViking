...
- The data source is the exact archive returned by commit: its `.done` marker is polled through `GET /content/read` (15 times at 2-second intervals), then its Working Memory is read once through `GET /content/overview`. This prevents either an earlier completed overview or a partial Phase 2 result from advancing the new boundary without repeatedly fetching a terminal archive whose overview is unavailable. The state is persisted in Pi's own branch via the custom entry `ov-takeover`.
